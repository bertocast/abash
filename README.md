...
Recent behavior lifts: `grep` now supports regex search plus narrow `-E`, `-F`, `-i`, `-n`, `-v`, `-c`, `-l`, and `-r`; `ln` now creates hard links by default and still supports `-s` for symlinks; `jq`/`yq` now support literals, binary ops, `select`, `map`, `length`, `type`, `keys`, `has`, and array/object construction.
