...
Recent behavior lifts: `grep` now supports regex search plus narrow `-E`, `-F`, `-i`, `-n`, `-v`, `-c`, `-l`, and `-r`; `ln` now creates hard links by default and still supports `-s` for symlinks; `jq` now supports literals, binary ops, `select`, `map`, `length`, `type`, `keys`, `has`, and array/object construction; `yq` now carries that jq-lite surface across YAML/JSON/TOML/CSV/INI/XML, plus front-matter extraction; `xan` now covers core row/column operations like `head`, `tail`, `slice`, `reverse`, `drop`, `rename`, and `enum`; `awk` now supports `BEGIN`/`END`, `-v`, variables, assignments, arithmetic, and basic comparisons.
...
- Supported today: simple commands, quoting, comments, `|`, `<`, `>`, `>>`, `2>`, `2>>`, `2>&1`, `;`, `&&`, `||`, `if ...; then ...; fi`, `if ...; then ...; elif ...; then ...; else ...; fi`, `while ...; do ...; done`, command-local assignment prefixes, `$NAME`, `${NAME}`, `${NAME:-default}`, `$1`, `$2`, `$@`, `$#` expansion, and argument globbing with `*`, `?`, and bracket classes.
...
- Unsupported today: global shell variables, broader control flow beyond minimal `if`/`elif`/`while` branches, functions, subshells, broader fd juggling beyond `2>`, `2>>`, `2>&1`, job control, and TTY semantics.
