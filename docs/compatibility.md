...
- narrow `yq` with YAML default input/output, optional JSON/TOML/CSV transcoding, file-extension auto-detect for `.json`/`.toml`/`.csv`/`.tsv`, and the current jq-lite filter surface: supported
