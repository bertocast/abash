...
- narrow `awk` with optional `-F`, `-v`, `BEGIN`/`END`, `print`, `printf`, variables, scalar and array assignments, arithmetic, regex literals, `$0/$N`, `NF/NR/FNR/FILENAME`, and basic comparisons / `~` / `!~`: supported
...
- narrow `yq` with YAML default input/output, optional JSON/TOML/CSV/INI/XML transcoding, file-extension auto-detect for `.json`/`.toml`/`.csv`/`.tsv`/`.ini`/`.xml`, `--front-matter`, narrow `-i` file rewrites, and the current jq-lite filter surface: supported
...
- narrow `xan` CSV toolkit with `headers`, `count`, `head`, `tail`, `slice`, `reverse`, `behead`, `cat`, `select`, `drop`, `rename`, `enum`, `search`, `sort`, `dedup`, `top`, `frequency`, `stats`, and `filter`: supported
